"""
基础组：对 train 集 blur_gamma 做均值滤波，相对 sharp（GT）计算 PSNR / SSIM 与耗时。
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="GOPRO train blur_gamma 均值滤波 PSNR/SSIM 评估")
    parser.add_argument(
        "--data-root",
        type=Path,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path(__file__).resolve().parent / "results" / "mean_filter.csv",
        help="逐张结果 CSV 路径",
    )
    parser.add_argument("--ksize", type=int, default=5, help="均值滤波核大小（奇数，默认 5）")
    ...
    for i, (blur_path, sharp_path, seq_name) in enumerate(pairs, 1):
        ...
        restored = mean_filter(blur, args.ksize)
    ...
    print(f"\n========== mean filter (ksize={args.ksize}) vs sharp (GT) ==========")
