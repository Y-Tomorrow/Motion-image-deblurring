"""
基础组：对 train 集 blur_gamma 做高斯滤波，相对 sharp（GT）计算 PSNR / SSIM 与耗时。
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="GOPRO train blur_gamma 高斯滤波 PSNR/SSIM 评估")
    parser.add_argument(
        "--data-root",
        type=Path,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path(__file__).resolve().parent / "results" / "gaussian_filter.csv",
        help="逐张结果 CSV 路径",
    )
    parser.add_argument("--ksize", type=int, default=5, help="高斯核大小（奇数，默认 5）")
    parser.add_argument("--sigma", type=float, default=0.0, help="高斯标准差（0 表示由 ksize 自动计算）")
    ...
    for i, (blur_path, sharp_path, seq_name) in enumerate(pairs, 1):
        ...
        restored = gaussian_filter(blur, args.ksize, args.sigma)
    ...
    print(f"\n========== gaussian filter (ksize={args.ksize}, sigma={args.sigma}) vs sharp (GT) ==========")
